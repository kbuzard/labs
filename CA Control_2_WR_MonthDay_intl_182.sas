*libname x '/sasr/Scavette/CA Good/Data';
%let dir = G:\MAX-Filer\Collab\Labs-kbuzard-S18\Admin;
libname x "&dir\Patents\CA Good\Data";


/*Import Inventor Dataset from Harvard Dataverse*/

proc import datafile="&dir\Patents\inventors5s_7595.tab"
     out=inventors
     dbms=dlm
     replace;
     delimiter='09'x;
...
proc import datafile="&dir\Patents\inventors5s_9608.tab"
     out=inventors_2
     dbms=dlm
     replace;
     delimiter='09'x;
...
PROC IMPORT OUT= WORK.pat76_06_assg 
            DATAFILE= "G:\MAX-Filer\Collab\Labs-kbuzard-S18\Admin\Bob_Hunt2\pat76_06_assg.dta"  
            DBMS=DTA REPLACE;
RUN;
proc sort;
by patent;
...
data controls_cites (keep = patent cite1--cite772);
array cite(1000);
...
PROC EXPORT DATA= X.possiblenclass 
            OUTFILE= "G:\MAX-Filer\Collab\Labs-kbuzard-S18\Admin\Patents
\RSUE\SASpossiblenclassCA.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
PROC EXPORT DATA= CITATIONS 
            OUTFILE= "G:\MAX-Filer\Collab\Labs-kbuzard-S18\Admin\Patents\RSUE\SAScitationsCA.csv" 
            DBMS=CSV REPLACE;
     PUTNAMES=YES;
RUN;
...
data x.matching (drop=x_citat x_control cite1--cite772);
	set matching;
run;
