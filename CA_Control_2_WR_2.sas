libname x 'H:\Jerry\Bob Hunt\CA SAS\Data';

/*Import Inventor Dataset from Harvard Dataverse*/

proc import datafile='H:\Jerry\California\Adam SAS Code\inventors5s_7595.tab'
     out=inventors
     dbms=dlm
     replace;
     delimiter='09'x;
...
proc import datafile='H:\Jerry\California\Adam SAS Code\inventors5s_9608.tab'
     out=inventors_2
     dbms=dlm
     replace;
     delimiter='09'x;
...
PROC IMPORT OUT= WORK.pat76_06_assg 
            DATAFILE= "X:\CARR\Tony Smith\Patents Update\NBER\pat76_06_assg.dta" 
            DBMS=DTA REPLACE;
RUN;
proc sort;
by patent;
...
data possiblenclass (keep=patent appyear gyear nclass pdpass invnum);
retain patent appyear gyear nclass pdpass invnum;
...
PROC IMPORT OUT= WORK.cite76_06
            DATAFILE= "X:\CARR\Tony Smith\Patents Update\NBER\cite76_06.dta" 
            DBMS=DTA REPLACE;
RUN;
proc sort;
by citing cited;
...
data citations;
	retain cited patent o_pdpass pdpass o_appyear o_gyear appyear gyear nclass;
...
data matching;
	...
	/*Statement below is 2nd date restriction 
	(Difference between Application Date of citing 
	and potential control <= 3 years)*/
	if abs(appyear-c_appyear)=<3;
run;
